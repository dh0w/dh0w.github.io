...
async function loadFigletFonts() {
  ...
  try {
    // Load Ogre font
    const ogreResponse = await fetch('Ogre.flf');
    const ogreFont = await ogreResponse.text();
    figlet.parseFont('Ogre', ogreFont);
    
    // Load Slant font
    const slantResponse = await fetch('Slant.flf');
    const slantFont = await slantResponse.text();
    ...
    alert('Error loading fonts. Make sure Ogre.flf and Slant.flf are in the same folder as your HTML file.');
    loadingFonts = false;
  }
}
...
async function convertTextToAscii() {
  ...
  if (!fontsLoaded) {
    ...
    if (!fontsLoaded) {
      return; // Error already shown in loadFigletFonts
    }
  }
...
// Initialize - load fonts when page loads
